...
class TrajectoryNode(Node):
    ...
    def __init__(self):
        ...
        # subscribers
        self.create_subscription(Odometry, '/zed/zed_node/odom', self.cb_zed, 10)
        self.create_subscription(Odometry, '/imu_state', self.cb_imu, 10)
        self.create_subscription(Odometry, '/filtered_state', self.cb_filtered, 10)
        self.create_subscription(Odometry, '/unicycle_dynamics', self.cb_uni, 10)
    ...
    def cb_zed(self, msg):
        self.zed_x.append(msg.pose.pose.position.x)
        self.zed_y.append(msg.pose.pose.position.y)
    ...
    def tf_update(self):
        try:
            ...
            self.tf_x.append(t.transform.translation.x)
            self.tf_y.append(t.transform.translation.y)
...
class TrajectoryWidget(Plugin):
    ...
    def __init__(self, context):
        ...
        for cb in (self.zed_checkbox, self.imu_checkbox, self.filt_checkbox,
                   self.tf_checkbox, self.uni_checkbox):
            cb.setChecked(True)
    ...
    def timerEvent(self, event):
        ...
        # ---- update plots (skip hidden ones to save work) ---- #
        if self.zed_curve.isVisible() and len(self.node.zed_x) > 1:
            self.zed_curve.setData(self.node.zed_x, self.node.zed_y)

        if self.imu_curve.isVisible() and len(self.node.imu_x) > 1:
            self.imu_curve.setData(self.node.imu_x, self.node.imu_y)

        if self.filt_curve.isVisible() and len(self.node.filt_x) > 1:
            self.filt_curve.setData(self.node.filt_x, self.node.filt_y)

        if self.tf_curve.isVisible() and len(self.node.tf_x) > 1:
            self.tf_curve.setData(self.node.tf_x, self.node.tf_y)

        if self.uni_curve.isVisible() and len(self.node.uni_x) > 1:
            self.uni_curve.setData(self.node.uni_x, self.node.uni_y)
